...
public class OpenCsvUtil {
	public static List<Customer>parseCsvFile(InputStream is){
		...
		try {
			...
			csvToBean=new CsvToBeanBuilder<Customer>(fileReader).withMappingStrategy(mappingStrategy).withSkipLines(1).withIgnoreLeadingWhiteSpace(true).build();
	...
	public static void customersToCsv(Writer writer, List<Customer> customers) {
		...
		beanToCsv = new StatefulBeanToCsvBuilder<Customer>(writer).withMappingStrategy(mappingStrategy).withQuotechar(CSVWriter.NO_QUOTE_CHARACTER).build();
